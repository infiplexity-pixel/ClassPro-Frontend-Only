...
/**
 * Fully simulate a browser loading the login page to seed CSRF tokens.
 */
...
async function fetchCaptchaImage(client, jar, cdigest) {
...
async function getSession(client, jar, password, identifier, digest) {
  ...
  // SI303 = "too many active sessions", need to clean up old ones
  if (code === 'SI303') {
...
/**
 * Perform the full two-step Zoho login flow.
 * @param {Object} opts - { username, password, captcha?, cdigest? }
  ...
  if (errors.length > 0) {
    ...
    if (requiresHIP(data)) {
      ...
      if (cdigestStr) {
        ...
        try {
          captchaImage = await fetchCaptchaImage(client, jar, cdigestStr);
        } catch (_) {}
        return {
          authenticated: false,
          session: null,
...
async function logout(cookie) {
  ...
  await client.get(ZOHO_LOGOUT_URL, {
    headers: {
      DNT: '1',
